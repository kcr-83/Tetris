...
namespace Tetris.Core.UI
{
    ...
    public static class TetrisGameExtensions
    {
        ...
        public static async Task RunGameLoopWithBetterResponsivenessAsync(this TetrisGame game, GameEngine gameEngine, 
            GameplayInterface gameplayInterface, bool isPaused, bool isGameActive, bool isExiting)
        {
            Console.Clear();
            ...
            while (isGameActive && !isExiting)
            {
                ...
                var checkForResizeMethod = gameplayInterface.GetType().GetMethod("CheckForResizeImproved");
                ...
                else
                {
                    ...
                    {
                        var field = gameplayInterface.GetType().GetField("_lastWindowWidth", 
                    ...
                    {
                        var field = gameplayInterface.GetType().GetField("_lastWindowHeight", 
                            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                        return field != null ? (int)field.GetValue(gameplayInterface) : 0;
                    }
                ...
                if (!isPaused)
                {
                    ...
                    {
                        ProcessGameInput(game, gameEngine, gameplayInterface, ref isPaused, ref isGameActive, ref isExiting);
                        inputsProcessed++;
                    }
                ...
                else
                {
                    ...
                    if (Console.KeyAvailable)
                    {
                        ...
                        {
                            PromptExitToMenu(game, gameplayInterface, ref isPaused, ref isGameActive);
                        }
                    }
                }
        ...
        /// </summary>
        public static async Task RunEnhancedGameLoopAsync(this TetrisGame game, GameEngine gameEngine, 
            GameplayInterfaceComplete gameplayInterface, bool isPaused, bool isGameActive, bool isExiting)
        {
            Console.Clear();
            Console.CursorVisible = false;
            ...
            while (isGameActive && !isExiting)
            {
                ...
                if (!isPaused)
                {
                    ...
                    while (Console.KeyAvailable && inputsProcessed < maxInputProcessingPerFrame)
                    {
                        ...
                        HandleSpecialKeysForResponsiveInterface(game, key, gameplayInterface, ref isPaused, ref isGameActive, ref isExiting);
        ...
        public static async Task RunWithResponsiveInterfaceAsync(this TetrisGame game)
        {
            ...
            var gameEngineField = game.GetType().GetField("_gameEngine", 
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            ...
            await RunEnhancedGameLoopAsync(game, gameEngine, gameplayInterfaceComplete, isPaused, isGameActive, isExiting);
        }
        ...
        /// </summary>
        private static void ProcessGameInput(TetrisGame game, GameEngine gameEngine, GameplayInterface gameplayInterface, 
            ref bool isPaused, ref bool isGameActive, ref bool isExiting)
        {
            var key = Console.ReadKey(true);
            bool fastDropActive = false;            // Try to use the improved input processing method if available
            var processGameInputMethod = gameplayInterface.GetType().GetMethod("ProcessGameInputImproved");
            ...
            else
            {
                ...
                switch (key.Key)
                {
                    ...
                    case ConsoleKey.Z:
                        ...
                        var counterClockwiseMethod = gameEngine.GetType().GetMethod("RotatePieceCounterClockwise");
        ...
        /// </summary>
        private static void HandleSpecialKeys(TetrisGame game, ConsoleKeyInfo key, 
            ref bool isPaused, ref bool isGameActive, ref bool isExiting)
        {
            switch (key.Key)
            {
        ...
        private static void TogglePause(TetrisGame game, ref bool isPaused)
        {
            ...
            var displayMethod = game.GetType().GetMethod("DisplayGame", 
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        ...
        /// </summary>
        private static void PromptExitToMenu(TetrisGame game, GameplayInterface gameplayInterface, 
            ref bool isPaused, ref bool isGameActive)
        {
            isPaused = true;
            ...
            Console.ForegroundColor = ConsoleColor.White;            bool answered = false;
            while (!answered)
            {
                var key = Console.ReadKey(true);
        ...
        /// </summary>
        private static void PromptExitMenuEnhanced(TetrisGame game, GameplayInterfaceComplete gameplayInterface, 
            ref bool isPaused, ref bool isGameActive)
        {
            isPaused = true;
        ...
        /// </summary>
        private static void HandleSpecialKeysForResponsiveInterface(TetrisGame game, ConsoleKeyInfo key, 
            GameplayInterfaceComplete gameplayInterface, ref bool isPaused, ref bool isGameActive, ref bool isExiting)
        {
            switch (key.Key)
            {
                case ConsoleKey.P:
                ...
                case ConsoleKey.Escape:
                    if (isPaused)
                    {
                        PromptExitMenuEnhanced(game, gameplayInterface, ref isPaused, ref isGameActive);
                    }
                    else
                    {
                        isPaused = true;
        ...
        public static TetrisGame EnableResponsiveInterface(this TetrisGame game)
        {
            ...
            var mainMenuField = game.GetType().GetField("_mainMenu", 
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                ...
                if (mainMenu != null)
                {
                    ...
                    if (eventField != null)
                    {
                        ...
                        EventHandler handler = async (sender, e) =>
                        {
                            ...
                            var isActiveField = game.GetType().GetField("_isGameActive", 
                                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
